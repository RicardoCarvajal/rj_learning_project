...
@Slf4j
@Component
public class AppFunction implements Function<List<RequestDto>, ResponseEntity<ResponseDto>> {
    ...
    public AppFunction(PropertiesConf properties, Utils utils, RestTemplate restTemplate) {
    ...
    public ResponseEntity<ResponseDto> apply(List<RequestDto> request) {
        try {
            List<String> result = request.stream().map(dto -> {
